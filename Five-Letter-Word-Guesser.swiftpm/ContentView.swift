...
struct ContentView: View {
    ...
    var body: some View {
        VStack {
            Text("Level \(wins + 1)")
                .font(.title2) // Display level with appropriate increment
                .fontWeight(.bold)
            ...
            HStack {
                Text("Wins: \(wins)")
                    .foregroundColor(Color.green) // Set color to green for wins
                Spacer()
                Text("Strikes: \(strikes)")
                    .foregroundColor(Color.red) // Set color to red for strikes
            ...
            if displaySpecialMessage { // Only show special message if flag is true
                Text(specialMessage)
                    .padding()
            }
    ...
    // Function to convert word to display
